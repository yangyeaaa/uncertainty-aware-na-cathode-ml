...
def main():
    ...
    for seed in SEEDS:
        ...
        for fold, (tr, va) in enumerate(kf.split(np.arange(n))):
            ...
            scaler = StandardScaler().fit(Y_all[tr])       # per-fold, leak-free
            dl_tr = DataLoader(DS(tr, scaler.transform(Y_all[tr]).astype(np.float32)),
                               batch_size=BATCH_SIZE, shuffle=True,
                               collate_fn=collate)
            dl_va = DataLoader(DS(va, scaler.transform(Y_all[va]).astype(np.float32)),
            ...
            preds = []
            with torch.no_grad():
                for g, lg, lat, _ in dl_va:
                    if not GRAPH_CPU:
                        g, lg = g.to(device), lg.to(device)
                    preds.append(model([g, lg, lat.to(device)]).cpu().numpy())
            oof[va] = scaler.inverse_transform(np.vstack(preds))
