...
# ============================================================
def train_one_fold(train_ds, es_ds, test_ds, n_targets, target_scaler):
    # v3: train_ds训练, es_ds早停(内部验证), test_ds仅最终评估(held-out fold)
    ...
    preds, trues = [], []
    with torch.no_grad():
        for els, fracs, y in test_loader:
...
def cross_validate(df):
    ...
    for fold, (trainval_idx, test_idx) in enumerate(kf.split(df), 1):
        ...
        # test_df = held-out测试折(仅评估); trainval内部再切train/es
        ...
        train_df = trainval_df.iloc[perm[n_es:]].reset_index(drop=True)
        es_df = trainval_df.iloc[perm[:n_es]].reset_index(drop=True)

        # ★ target_scaler只在真正训练子集上fit
        target_scaler = StandardScaler()
        target_scaler.fit(train_df[TARGETS].values)

        train_ds = NaCathodeDataset(train_df, TARGETS)
        es_ds = NaCathodeDataset(es_df, TARGETS)
        test_ds = NaCathodeDataset(test_df, TARGETS)

        torch.manual_seed(SEED + fold)
        trues, preds, n_epochs = train_one_fold(
            train_ds, es_ds, test_ds, n_targets, target_scaler
        )
