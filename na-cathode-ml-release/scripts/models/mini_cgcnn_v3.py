...
# ============================================================
def train_one_fold(train_ds, es_ds, test_ds, target_scaler):
    """
    ★ 关键修复(v3): 消除测试折泄漏.
       - train_ds : 用于梯度更新的训练子集
       - es_ds    : early-stopping 验证子集(从训练折内部再切出), 只用于选 checkpoint
       - test_ds  : held-out 测试折, 只用于最终评估, 不参与任何模型选择
       target_scaler 由外部在 train_ds 上 fit 后传入.
    """
    train_loader = DataLoader(train_ds, batch_size=BATCH_SIZE,
                              shuffle=True, num_workers=0)
    # early-stopping 验证集(内部验证, 决定何时停/选哪个checkpoint)
    es_loader = DataLoader(es_ds, batch_size=BATCH_SIZE,
                           shuffle=False, num_workers=0)
    # 测试集(held-out fold, 仅最终评估)
    ...
    for epoch in range(EPOCHS):
        ...
        # 早停用 es_loader(内部验证集), 不碰 test_loader
        model.eval()
        es_loss, n_count = 0, 0
        with torch.no_grad():
            for batch in es_loader:
                ...
                pred = model(atom_z, nbr_idx, nbr_dist, mask)
                es_loss += loss_fn(pred, y_norm).item() * len(y)
                n_count += len(y)
        es_loss /= max(n_count, 1)
    ...
    # 最终评估在 test_loader(held-out fold)上, 该折从未参与早停
    model.eval()
    preds, trues = [], []
    with torch.no_grad():
...
def cross_validate(df, structures):
    ...
    for fold, (trainval_idx, test_idx) in enumerate(kf.split(df), 1):
        log(f"  Fold {fold}/{N_SPLITS} ...")
        # test_df = held-out 测试折(仅最终评估, 绝不参与早停)
        test_df = df.iloc[test_idx].reset_index(drop=True)

        # ★ v3关键: 从训练部分内部再切出 early-stopping 验证子集
        #    train : es = 90 : 10, 用固定种子保证可复现
        #    held-out 测试折完全不参与早停/模型选择
        trainval_df = df.iloc[trainval_idx].reset_index(drop=True)
        es_frac = 0.1
        rng = np.random.RandomState(SEED + fold)
        perm = rng.permutation(len(trainval_df))
        n_es = max(1, int(round(len(trainval_df) * es_frac)))
        es_sel = perm[:n_es]
        tr_sel = perm[n_es:]
        tr_df = trainval_df.iloc[tr_sel].reset_index(drop=True)
        es_df = trainval_df.iloc[es_sel].reset_index(drop=True)

        # ★ target_scaler 只在真正的训练子集 tr_df 上 fit
        target_scaler = StandardScaler()
        target_scaler.fit(tr_df[TARGETS].values)

        t_fold = time.time()
        tr_ds = CrystalDataset(tr_df, structures, TARGETS)
        es_ds = CrystalDataset(es_df, structures, TARGETS)
        test_ds = CrystalDataset(test_df, structures, TARGETS)
        log(f"    train n={len(tr_ds)}, es n={len(es_ds)}, "
            f"test n={len(test_ds)} (skipped tr={tr_ds._skipped})")

        torch.manual_seed(SEED + fold)
        trues, preds, n_epochs = train_one_fold(tr_ds, es_ds, test_ds, target_scaler)
        dt = time.time() - t_fold
